        ...
        for epoch_number in tqdm(range(args.number_epochs)):
            for epoch_id, iteration_number in enumerate(range(0, len(train_filenames), args.batch_size)):
                ...
                try:
                    ...
                    _, loss_value = sess.run([train_op, cross_entropy], feed_dict={is_training : True, x_train: train_imgs, y_train: train_labels})
                    train_counter += 1
            ...
            for iteration_number in range(0, len(val_filenames), args.batch_size):
                try:
                    ...
                    loss_value = sess.run([cross_entropy], feed_dict={is_training : False, x_train: val_imgs, y_train: val_labels})
